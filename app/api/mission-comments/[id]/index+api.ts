import { deleteMissionComment, updateMissionComment } from '@/src/backend/mission-comments';
...
export async function PATCH(
  ...
  return withRequestContext(request, async (ctx) => {
    ...
    const result = await updateMissionComment(ctx, id, body);

    if (!result.ok) {
      const status =
        result.code === 'mission_comment_not_found'
          ? 404
