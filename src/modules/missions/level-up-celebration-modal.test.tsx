...
describe('LevelUpCelebrationModal', () => {
  test('is hidden when newLevel is null', async () => {
    const view = await render(
      <LevelUpCelebrationModal newLevel={null} onClose={() => undefined} />,
    );

    expect(view.queryByText(/You reached Level/)).toBeNull();
  });

  test('shows the reached level when newLevel is set', async () => {
    const view = await render(
      <LevelUpCelebrationModal newLevel={7} onClose={() => undefined} />,
    );
  ...
  test('calls onClose when the acknowledge button is pressed', async () => {
    ...
    const view = await render(
      <LevelUpCelebrationModal newLevel={3} onClose={onClose} />,
    );
