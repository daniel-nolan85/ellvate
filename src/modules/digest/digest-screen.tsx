...
export function DigestScreen({ weekStart }: DigestScreenProps) {
  ...
  return (
    // This screen is always formSheet-presented (see app/_layout.tsx and
    // SHEET_SCREEN_OPTIONS), so unlike the plain-push screens elsewhere in
    // the app, `flex-1` on this root is not a reliable way to fill the
    // sheet's allocated detent height -- see SHEET_SCREEN_OPTIONS' own WHY
    // for the underlying react-native-screens quirk; contentStyle:
    ...
    <View className="bg-canvas" collapsable={false} style={{ height: '100%' }}>
      {/* No manual close button -- this screen is presented as a native
          ...
          it. Safe here even though react-native-screens#3569 (a formSheet-
          over-formSheet content-height bug this screen used to trigger on
          every notification tap) is why Notifications itself, and the
          formSheet duplicates it links to, stay separate from this screen's
          own presentation -- both of Digest's entry points (Notifications,
          a Profile icon tap) are plain screens, not formSheets, so nothing
          chains here. A formSheet's content starts well below the physical
          top edge, so a small fixed gap is enough, not insets.top -- unlike
          Sheet's statusBarTranslucent custom Modal, which spans behind the
          notch. collapsable={false} works around a real react-native-screens bug
